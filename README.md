...
Every send — right-click menu, left-click magnet, or an intercepted `.torrent`
download — opens a small window asking which qBittorrent category to file it
under. The list is read live from qBittorrent, the save path for the highlighted
category is shown, and the last category you used is pre-selected. Closing the
window (Cancel / Esc) sends nothing.

Because the category decides the save path, the prompt is deliberately not
optional: a torrent that must land in a specific folder cannot be added to the
default one by forgetting a step. Pick **— no category —** to add without one.

Categories must already exist in qBittorrent, with a save path set
(*Category → Add category*). The prompt only offers what qBittorrent reports.
